...
// demo2CEP demonstrates stateful CEP pattern matching.
//
...
func demo2CEP() {
	...
	ruleUC := ruleUsecase.NewRuleUsecase()
	proc := cepUsecase.NewCEPUsecase(store, ruleUC)
	...
	proc.AddPattern(depositThenWithdraw)
	...
	results1, err := proc.ProcessEvent(ctx, event1)
	...
	results2, err := proc.ProcessEvent(ctx, event2)
	fmt.Printf("After CryptoWithdraw (same addr):  matches=%d  err=%v\n", len(results2), err)
	for _, r := range results2 {
		fmt.Printf("  MATCH: pattern=%q  member=%s  vars=%v\n", r.PatternName, r.MemberID, r.Variables)
	}

	// New processor for clean state — withdraw to wrong address should not match
	store3 := cepMemory.NewMemoryStore()
	proc3 := cepUsecase.NewCEPUsecase(store3, ruleUsecase.NewRuleUsecase())
	...
	results3, err := proc3.ProcessEvent(ctx, event3)
	fmt.Printf("After CryptoWithdraw (wrong addr): matches=%d  err=%v\n", len(results3), err)
}
