...
func demo1AST() {
	...
	rule := ast.RuleNode{
		...
		Children: []ast.RuleNode{
			{
				Type:     ast.NodeCondition,
				Field:    "withdraw_count_3d",
				Operator: ">",
				Value:    float64(5),
				Window:   &ast.TimeWindow{Value: 3, Unit: "days"},
			},
			{
				Type: ast.NodeOr,
				Children: []ast.RuleNode{
					...
					},
					{
						...
						Window:   &ast.TimeWindow{Value: 1, Unit: "days"},
					},
				},
			},
		},
...
func demo2CEP() {
	...
	depositThenWithdraw := cep.CEPPattern{
		ID:   "pattern-deposit-withdraw",
		Name: "Deposit then Withdraw to Same Address",
		States: []cep.PatternState{
			{
				Name: "deposit",
				Condition: ast.RuleNode{
					Type:     ast.NodeCondition,
					Field:    "behavior",
					Operator: "=",
					Value:    "CryptoDeposit",
			...
			{
				Name:    "withdraw",
				MaxWait: &ast.TimeWindow{Value: 10, Unit: "minutes"},
	...
	store2 := cep.NewMemoryStore()
	eng2 := engine.New(store2)
