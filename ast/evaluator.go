...
func Evaluate(node RuleNode, ctx EvalContext) (bool, error) {
	switch node.Type {
	case NodeAnd:
...
func evalAnd(children []RuleNode, ctx EvalContext) (bool, error) {
...
func evalOr(children []RuleNode, ctx EvalContext) (bool, error) {
	for _, child := range children {
		ok, err := Evaluate(child, ctx)
		if err != nil {
			return false, err
...
func evalNot(children []RuleNode, ctx EvalContext) (bool, error) {
	if len(children) != 1 {
...
func evalCondition(node RuleNode, ctx EvalContext) (bool, error) {
...
func resolveRHS(value any, ctx EvalContext) (any, error) {
	s, ok := value.(string)
...
func resolveValue(field string, window *TimeWindow, ctx EvalContext) (any, error) {
